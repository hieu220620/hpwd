...
export const wishas = () => {
    ...
    const listItemBank = (data) => (
        `
        <figure class="bank-item" data-aos="zoom-in" data-aos-duration="1000">
            <img src="${data.qr}" alt="QR ${data.bankName}">

            <figcaption>
                <p class="bank-name">Ngân hàng: ${data.bankName}</p>
                <p class="account-name">Chủ tài khoản: <strong>${data.name}</strong></p>
                <p class="account-number">Số tài khoản: ${data.rekening}</p>
            </figcaption>
        </figure> `);
